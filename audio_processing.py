...
def heavy_remix(input_path: str, output_path: str) -> bool:
    ...
    try:
        ...
        # 2.5 Loop Short Audio (Transformative Requirement)
        duration = y.shape[1] / sr
        if duration < 15.0:
            logger.info(f"   🔄 Audio too short ({duration:.1f}s). Looping to >15s...")
            target_len = int(15.0 * sr)
            repeats = int(np.ceil(target_len / y.shape[1]))
            y = np.tile(y, (1, repeats))
            # Trim to exact target if needed, or just keep it looped
            # Let's keep it slightly longer to be safe
            logger.info(f"   ✅ New duration: {y.shape[1]/sr:.1f}s")
